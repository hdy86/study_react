...
const secretKey = "aldkjfoiejqof1234!@#";

const AuthController = {
  postAuthSignup: async (req, res) => {
    // 1. user_name 중복검사 : 유저가 보낸 user_name과 일치하는 값이 DB에 있는가?
    // 1-1. 있으면 회원가입 실패
    // 2. 비밀번호 암호화 : 평문과 salt(랜덤값)을 조합
  ...
  postAuthLogin: async (req, res) => {
    ...
    // 1-1. 없으면 로그인 실패
    // 2. 1.에서 가져온 salt와 유저가 입력한 비밀번호로 hashedPW 만들기
